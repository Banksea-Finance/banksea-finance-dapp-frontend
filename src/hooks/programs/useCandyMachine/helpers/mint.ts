...
import { CitizenOneCandyMachineAddress, TOKEN_METADATA_PROGRAM_ID, TOKEN_PROGRAM_ID, } from './constant'
...
export async function mint(program: Program, mint: Keypair): Promise<MintResult> {
  ...
  const candyMachine: any = await program.account.candyMachine.fetch(CitizenOneCandyMachineAddress)
  ...
  const [candyMachineCreator, creatorBump] = await getCandyMachineCreator(CitizenOneCandyMachineAddress)

  instructions.push(
    await program.instruction.mintNft(creatorBump, {
      accounts: {
        candyMachine: CitizenOneCandyMachineAddress,
        candyMachineCreator,
